...
O3D_IMPLEMENT_DYNAMIC_CLASS1(DepthPassMaterial, ENGINE_DEPTH_PATH_MATERIAL, Material)

// Default constructor.
DepthPassMaterial::DepthPassMaterial(BaseObject *parent) :
	Material(parent),
	m_opacityMap(False)
{
	m_shaderName = "depthPass";
	m_name = "depthPass";
	m_initMode = AMBIENT;
}

// Virtual destructor.
DepthPassMaterial::~DepthPassMaterial()
{

}

// Check if the material is supported by the hardware.
Bool DepthPassMaterial::isMaterialSupported() const
{
	return True;
}

// Initialize material shader according to a specific MaterialPass setting.
void DepthPassMaterial::initialize(
		InitMode initMode,
...
void DepthPassMaterial::release()
{
...
void DepthPassMaterial::processAmbient(
		Shadable &object,
...
void DepthPassMaterial::processPicking(
		Shadable &object,
		Pickable &pickable,
		const DrawInfo &drawInfo,
		const MaterialPass &materialPass)
...
void DepthPassMaterial::processLighting(
		Shadable &object,
...
void DepthPassMaterial::processDeferred(
		Shadable &object,
		const DrawInfo &drawInfo,
		const MaterialPass &materialPass)
{
