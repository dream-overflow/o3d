...
void format_uuid_v3or5(uuid_t *uuid, unsigned char hash[16], int v)
{
    ...
    // memcpy(uuid, hash, sizeof *uuid); fixed by following
    uuid->clock_seq_hi_and_reserved =  hash[8];
    uuid->clock_seq_low = hash[9];
    memcpy(uuid->node, &hash[10], 6);
