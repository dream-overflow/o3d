...
void main()
{
	...
	float noise1 = texture(u_noise, u_noiseParams[0] * io_texCoords).r - 0.5;
	float noise2 = texture(u_noise, u_noiseParams[1] * io_texCoords).r - 0.5;
	float noise3 = texture(u_noise, u_noiseParams[2] * io_texCoords).r - 0.5;
	...
	float intensity = texture(u_cloud, io_texCoords).r;
