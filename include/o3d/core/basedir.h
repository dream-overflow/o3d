...
#ifndef _O3D_BASEDIR_H
#define _O3D_BASEDIR_H
...
/**
 * @brief Abstract class for manager directory. @see Dir for file system directory management.
 * Path are returned without trailing slash. Any trailing slash or backslash
 * are removed the given path name.
 */
class O3D_API BaseDir
{
public:
    ...
    BaseDir(const String& pathname = String());

	//! copy constructor
    BaseDir(const BaseDir& dup) :
		m_type(dup.m_type),
    ...
    virtual BaseDir::DirReturn check(const String &fileOrPath) const = 0;
    ...
    virtual BaseDir::DirReturn removeFile(const String &filename) const = 0;

	//! rename a file or a directory
    virtual BaseDir::DirReturn rename(const String &oldName, const String &newName) const = 0;

	//! copy the given absolute filename/relative to working directory, to this dir
    virtual BaseDir::DirReturn copyFile(const String &filename, UInt32 BlockSize = 32768) const = 0;

	//! check if two dir are the same
    inline Bool operator== (const BaseDir &cmp) const
	{
