...
//! @class FileInfo
//-------------------------------------------------------------------------------------
//! File information object. It take informations available in stats, truncates
//! strings path and file name. Its a very useful class to avoid you of some basics
//! repetitive calls. This is the abstract class (see DiskFileInfo for file system one)
//---------------------------------------------------------------------------------------
class O3D_API FileInfo
{
protected:
...
public:
	...
	enum FileInfoType
	{
		FILE_SYSTEM_FILE_INFO,  //!< Local file system or O3DZip virtual file system.
	...
	//! constructor with separates pathname and filename
    FileInfo(const Dir &dir, const String &filename);

	//! copy constructor
	FileInfo(const FileInfo& dup);

	//! duplicator (take care it copy the state of the cached data)
	inline FileInfo& operator=(const FileInfo& dup)
	{
		m_type = dup.m_type;
	...
	inline String makeRelative(const Dir &dir)const
	{
