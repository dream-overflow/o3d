...
// Initialize manually.
Date::Date(Int16 _year, Int8 _month, Int8 _day, Int8 _dayofWeek, Int8 _tz):
    month(_month),
    wday(_day),
    mday(_dayofWeek),
    year(_year),
...
void Date::fromTimeUs(Int64 us, Bool UTC)
{
    ...
    if (UTC) {
        ...
        local = localtime(&ts);
        tz = (Int8)(sm_localTz / 3600);  // @todo dst
    }
...
time_t Date::toTime_t(Bool UTC) const
{
    ...
    lObjectTime.tm_sec = 0;  // (UTC ? sm_localTz : 0);
...
Int8 Date::getIsoDayOfWeek() const
{
...
Int8 Date::getDayOfWeek() const
{
    ...
    // not correction for 0..6 starting on monday
