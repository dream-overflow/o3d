...
//! @class IsoSphere
//-------------------------------------------------------------------------------------
//! Definition of an iso-sphere primitive
//---------------------------------------------------------------------------------------
...
public:
	...
	//! Is half sphere
	Bool isHalfSphere() const { return m_capacities & HALF_SPHERE; }
...
protected:
	...
	Float m_radius;  //<! IN. the radius of the sphere

	UInt32 m_subDiv;   //<! IN. specify how many times you want to subdivide the icosahedron.
					 	 //<! The vertex number is given by:  V(subDiv) = V(n) = 2+10*2^(2*n) for a whole sphere
						 //<!								 V(n) = 1+5.2^(n-1)*(2^(n+1)+1) for a half sphere

	Bool m_halfSphere;  //<! IN. true if you want only a half sphere
	Bool m_optimize;    //<! Not supported yet
};
